...
long_load_2 <- long_load_2 %>% 
  filter(Date !='2019-09-20') %>% 
  filter(Date!='2019-07-22' & site_res!='FCR_200')

#########################################
## set sp cond at stream sites to be the same as first in-reservoir site to test robustness of processing metric
long_load_2$value[long_load_2$variable=='Sp_cond_uScm' & long_load_2$Site=='100' & long_load_2$Reservoir=='BVR'] <- long_load_2$value[long_load_2$variable=='Sp_cond_uScm' & long_load_2$Site=='20' & long_load_2$Reservoir=='BVR']
long_load_2$value[long_load_2$variable=='Sp_cond_uScm' & long_load_2$Site=='200' & long_load_2$Reservoir=='BVR'] <- long_load_2$value[long_load_2$variable=='Sp_cond_uScm' & long_load_2$Site=='30' & long_load_2$Reservoir=='BVR']

long_load_2$value[long_load_2$variable=='Sp_cond_uScm' & long_load_2$Site=='200' & long_load_2$Reservoir=='FCR'] <- long_load_2$value[long_load_2$variable=='Sp_cond_uScm' & long_load_2$Site=='20' & long_load_2$Reservoir=='FCR']
long_load_2$value[long_load_2$variable=='Sp_cond_uScm' & long_load_2$Site=='99' & long_load_2$Reservoir=='FCR'] <- long_load_2$value[long_load_2$variable=='Sp_cond_uScm' & long_load_2$Site=='20' & long_load_2$Reservoir=='FCR']
...
ggplot(data = test_load[test_load$variable=='Sp_cond_uScm',], aes(x = distance_m, y = value)) +
  facet_wrap(~Reservoir, scales = 'free_x') +
  geom_line(aes(col = as.factor(month(Date))), size = 2) +
